...
def generar_comentario(puntaje, test_nombre):
    try:
        prompt_usuario = (
            f"El usuario ha completado el test '{test_nombre}', que evalúa ansiedad social. "
            f"Obtuvo un puntaje de {puntaje}. ¿Qué comentario cálido y alentador le darías?"
        )

        response = client.chat.completions.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "Eres un asistente psicológico empático y profesional. Tu tarea es ofrecer comentarios comprensivos, claros y motivadores para personas que acaban de realizar un test de ansiedad social."},
        ...
        return "Hubo un error al generar tu comentario emocional. Intenta más tarde."
